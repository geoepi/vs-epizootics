...
## Phylogeny
```{r}
Vs.Tree = read.tree(here("assets/mcc.tre"))

ggtree(Vs.Tree.c, mrsd="2016-01-10", as.Date = F, size=0.5, col = "gray30") + 
  ggnewscale::new_scale_colour()  + 
  geom_range(range="height_0.95_HPD", color= "bisque3", alpha=.4, size=1.8) +
...
VS_Coal_pref <- phylodyn::BNPR_PS(Vs.Tree, lengthout = 100, prec_alpha = 0.01, prec_beta = 0.01,
                   beta1_prec = 0.001, fns = NULL, log_fns = FALSE, simplify = TRUE,
                   derivative = FALSE, forward = TRUE)

phylodyn::plot_BNPR(VS_Coal_pref)

VS_Coal_pref.df <- as.data.frame(
                        cbind(
                          Ne = VS_Coal_pref$effpop,
                          Ne.low = VS_Coal_pref$effpop025,
                          Ne.high = VS_Coal_pref$effpop975))

VS_Coal_pref.df$date <- as.Date("2016-01-10") - days(round(VS_Coal_pref$x*365.25,0))

VS_Coal_pref.df = arrange(VS_Coal_pref.df, desc(date))


ggplot(VS_Coal_pref.df, aes(date, Ne)) +
  geom_line()
```
...
# Run Model
```{r}
#theta1 = Model.demo$internal.summary.hyperpar$mean
theta1 = c(2.991771, 6.239724, -2.263282, -4.036796, -1.306113)   
...
## Fitted Estimates
```{r}
indx <- inla.stack.index(joint.stk, "est.0")$data
combined_data$fitted = Model.demo$summary.fitted.values$mean[indx]

ggplot(combined_data, aes(obs, fitted)) +
  geom_point() +
  geom_line()
...
plot_rast_panels(pf_stack.r, domain, join_sims, rev(coolwarm(4)), center=TRUE)
```
